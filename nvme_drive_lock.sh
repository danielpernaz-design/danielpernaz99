...
encrypt_drive() {
    ...
    { CURRENT=$(sedutil-cli -n --query "$DEVICE" 2>/dev/null | tr -d '\0'); } 2>/dev/null
    ...
    { QUERY_OUTPUT=$(sedutil-cli -n --query "$DEVICE" | tr -d '\0'); } 2>/dev/null
    echo "$QUERY_OUTPUT"
...
show_status() {
    ...
    { QUERY_OUTPUT=$(sedutil-cli -n --query "$DEVICE" 2>/dev/null | tr -d '\0'); } 2>/dev/null
